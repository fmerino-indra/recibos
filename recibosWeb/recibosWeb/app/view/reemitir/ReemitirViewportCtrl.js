Ext.define('recibosWeb.view.reemitir.ReemitirViewportCtrl', {
    extend  : 'Ext.app.ViewController',
    alias   : 'controller.reemitir-viewport',
    requires: [
    ...
    control: {
        'reemitir_reemitirgrid'                 : {
            itemdblclick   : 'reemitirOnDataListDblClick'
//            ,
//            selectionchange: 'reemitirOnDatalistSelectionChange'
        },
        'ux-maintoolbar button'               : {
            click: 'onAction'
        },
        'reemitir_reemitirmaster ux-maintoolbar': {
...
/*      TODO FMM
     ...
     * @param view
     */
    reemitirOnDataListDblClick: function (view, model) {
        var me = this,
            master = me.lookupReference("reemitir_reemitirdetailmaster"),
            detail = master.down('reemitir_reemitirdetailform'),
            detailGrid = master.down('reemitir_reemitirdetailgrid'),
        ...
        dependencies = detailController.reemitirReloadDependentStores();

        RSVP.all(dependencies).then(function () {
            detailFormController.reemitirViewDetail(model);
        });
    ...
    newRecord: function (btnNewRecord) {
        var me = this, viewport = me.getView(), layout = viewport
            .getLayout(), detail = viewport.down('reemitir_reemitirdetailform'), form = detail
            .getForm();

        if (layout.getActiveItem() === viewport.items.getAt(0)) {
            layout.setActiveItem(1);
        }
        detail.setTitle(t('reemitir.detail.title'));
        form.reset();
    ...
    editRecord: function (btnEditRecord) {
        ...
        master = btnEditRecord.up(me.getView()).down(
            'reemitir_reemitirmaster');
        rows = master.getSelection();

        if (rows && rows.length > 0) {
            me.reemitirOnDataListDblClick(btnEditRecord, rows[0]);
        } else {
            var wObject = {
    ...
    back      : function () {
        var me = this;
        me.getView().down('reemitir_reemitirmaster').getController().reemitirLoad();
    ...
    deleteRecord: function (btnDeleteRecord) {
        var me = this, viewport = me.getView(), layout = viewport
            ...
            .down('reemitir_reemitirgrid'), store;
        ...
        me.log('Prepared to remove reemitir with id: "' + record.data.id
            + '"');
        Ext.MessageBox.confirm(t('commons.msgs.warning'),
            t('commons.msgs.question.delete'),
            function (action) {
                if (action === 'yes') {
                    me.reemitirDeleteAction(record, function () {
                        store.remove(record);
     ...
     */
    reemitirDeleteAction: function (record, fn) {
        var me = this;
        record.erase({
    ...
    getIdsFromSelection: function (getEnabled) {
        ...
        selection = me.lookupReference('reemitir_reemitirdetailmaster').down('reemitir_reemitirdetailgrid').getSelectionModel().getSelection();
...
//                me.getView().down('reemitir_reemitirmaster').getController().usuarioLoad();
                selection = me.getView().down('reemitir_reemitirgrid').getSelectionModel().getSelection();
                me.getView().down('reemitir_reemitirdetailmaster').getController().reemitirViewDetail(selection[0]);
            },
