Ext.define('recibosWeb.view.periodo.detail.PeriodoDetailCtrl', {
    extend: 'iDynamicsFront.util.ViewController',
    alias : 'controller.periodo-detail',

    control: {
        'periodo_periododetailform': {
            beforesaveform: 'periodoOnBeforeSave',
            saveform      : 'periodoSave'
        }
    },
     ...
     *            detail
     */
    periodoViewDetail: function (modelSelected) {
        var me = this, uxfieldArray, uxfieldNArray,
            uxfield, uxfieldN, boundList, len, len2, model, datos, detail, enabled, bt;

        detail = me.getView();
        recibosWeb.model.Periodo.load(modelSelected.get('id'), {
            callback: function (periodo, operation) {
                if (periodo.data === null) {
     ...
     */
    periodoReloadDependentStores: function (detail) {
        var me = this, viewModel = this.getViewModel(),
            stores = ['periodos'], storeLoader;
    ...
    periodoSave: function (saveBtn) {
        ...
        if (form.isValid()) {
            ...
            if (record.phantom) {
                //creo nuevo periodo, no envío id
                record.setId(null);
            }
            record.save({
