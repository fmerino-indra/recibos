Ext.define('recibosWeb.view.persona.detail.PersonaDetailCtrl', {
    extend: 'iDynamicsFront.util.ViewController',
    alias : 'controller.persona-detail',

    control: {
        'persona_personadetailform': {
            beforesaveform: 'personaOnBeforeSave',
            saveform      : 'personaSave'
        }
    },
     ...
     *            detail
     */
    personaViewDetail: function (modelSelected) {
        var me = this, uxfieldArray, uxfieldNArray,
            uxfield, uxfieldN, boundList, len, len2, model, datos, detail, enabled, bt;

        detail = me.getView();
        recibosWeb.model.Persona.load(modelSelected.get('id'), {
            callback: function (persona, operation) {
     ...
     */
    personaReloadDependentStores: function (detail) {
        var me = this, viewModel = this.getViewModel(),
            stores = ['personas'], storeLoader;
    ...
    personaSave: function (saveBtn) {
        ...
        if (form.isValid()) {
            ...
            if (record.phantom) {
                //creo nuevo persona, no envío id
                record.setId(null);
            }
            record.save({
