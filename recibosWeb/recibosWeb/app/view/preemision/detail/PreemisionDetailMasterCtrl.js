Ext.define('recibosWeb.view.preemision.detail.PreemisionDetailMasterCtrl', {
    extend: 'iDynamicsFront.util.ViewController',
    alias : 'controller.preemision-detailmaster',
    control: {
        'preemision_preemisiondetailform': {
//            beforesaveform: 'preemisionOnBeforeSave',
//            saveform      : 'preemisionSave'
        },
        'preemision_preemisiondetailgrid' : {
            selectionchange: 'onSelectionChange'
        }

    },
    preemisionViewDetailGrid: function (modelSelected) {
        var me = this, grid;
        grid = me.getView().down('preemision_preemisiondetailgrid');
     ...
     *            detail
     */
    preemisionViewDetail: function (modelSelected) {
        var me = this, uxfieldArray, uxfieldNArray,
            uxfield, uxfieldN, boundList, len, len2, model, datos, detail, enabled, bt;

        detail = me.getView().down('preemision_preemisiondetailform');
        recibosWeb.model.PreemisionDTO.load(modelSelected.get('id'), {
            callback: function (preemision, operation) {
                if (preemision.data === null) {
...
//                    me.getView().setTitle(t('preemision.detail.title')+ ' ('+);
                    ...
                    me.preemisionViewDetailGrid(model);
                }
            }
        });
    },
     ...
     */
    preemisionReloadDependentStores: function (detail) {
        var me = this, viewModel = this.getViewModel(),
            stores = ['preemisions'], storeLoader;
    ...
    hasAnyReturned: function (active) {
        ...
        selection = this.getView().down('preemision_preemisiondetailgrid').getSelectionModel().getSelection();
        if (selection.length) {
